...
describe('cleanupOrphans', () => {
  it('stops orphaned nanoclaw containers from JSON output', () => {
    ...
    const lsOutput = JSON.stringify([
      ...
      { status: 'running', configuration: { id: 'nanoclaw-testslug-group3-333' } },
      { status: 'running', configuration: { id: 'nanoclaw-other-group4-444' } },
    ...
    // ls + stop call for this install only
    expect(mockExecSync).toHaveBeenCalledTimes(2);
    expect(mockExecSync).toHaveBeenNthCalledWith(2, `${CONTAINER_RUNTIME_BIN} stop nanoclaw-testslug-group3-333`, {
      stdio: 'pipe',
    });
    expect(log.info).toHaveBeenCalledWith('Stopped orphaned containers', {
      count: 1,
      names: ['nanoclaw-testslug-group3-333'],
    });
  });
  ...
  it('continues stopping remaining containers when one stop fails', () => {
    const lsOutput = JSON.stringify([
      { status: 'running', configuration: { id: 'nanoclaw-testslug-a-1' } },
      { status: 'running', configuration: { id: 'nanoclaw-testslug-b-2' } },
    ]);
    mockExecSync.mockReturnValueOnce(lsOutput);
    // First stop fails
    mockExecSync.mockImplementationOnce(() => {
    ...
    expect(log.info).toHaveBeenCalledWith('Stopped orphaned containers', {
      count: 2,
      names: ['nanoclaw-testslug-a-1', 'nanoclaw-testslug-b-2'],
    });
  });
});
