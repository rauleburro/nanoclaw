...
2. Call `create_agent({ name: "Builder", instructions: "<builder prompt>" })` — the name becomes the destination for that builder
3. After the creation notification arrives, call `send_message({ to: "Builder", text: "<task description with specific files and changes>" })`
...
- **Report back.** When done, use `send_message` to tell the requesting agent: (a) what files you changed, (b) a summary of the changes, (c) any follow-up needed (rebuild, tests, migrations).
...
3. If nothing suitable exists → delegate to a builder agent:
   ...
   - `send_message({ to: "RSS Tool Builder", text: "Add an MCP tool 'read_rss' to container/agent-runner/src/mcp-tools/. It should fetch an RSS URL and return the latest N items. Register it in mcp-tools/index.ts. Target: <200 new lines." })`
